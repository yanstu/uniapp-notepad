		...
		>
			...
			>
				<view
					...
					v-if="mode == 'number'"
					:style="{
						backgroundColor: current < index ? 'transparent' : (item.activeColor?item.activeColor:activeColor),
						borderColor: current < index ? (item.unActiveColor?item.unActiveColor:unActiveColor): (item.activeColor?item.activeColor:activeColor)
					}"
				>
					<text v-if="current < index" :style="{
						color: current < index ? (item.unActiveColor?item.unActiveColor:unActiveColor) : (item.activeColor?item.activeColor:activeColor),
					}">
						{{ index + 1 }}
					</text>
					<u-icon v-else size="22" color="#ffffff" :name="(item.icon?item.icon:icon)"></u-icon>
				</view>
				<view class="u-steps__item__dot" v-if="mode == 'dot'" :style="{ 
					backgroundColor: index <= current ? (item.activeColor?item.activeColor:activeColor) : (item.unActiveColor?item.unActiveColor:unActiveColor) 
				}"></view>
				<text class="u-line-1" :style="{ 
					color: index <= current ? (item.activeColor?item.activeColor:activeColor) : (item.unActiveColor?item.unActiveColor:unActiveColor),
				...
				<view class="u-steps__item__line" :class="['u-steps__item__line--' + mode]" v-if="index < list.length - 1">
					<u-line :direction="direction" length="100%" :hair-line="false" :color="(item.unActiveColor?item.unActiveColor:unActiveColor)"></u-line>
				</view>
			</view>
		</view>
	</view>
